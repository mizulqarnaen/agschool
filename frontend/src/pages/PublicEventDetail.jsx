...
export const PublicEventDetail = () => {
  ...
  const getPaymentBadge = (status, date) => {
    ...
    switch (status) {
      case 'Paid':
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-semibold rounded-full bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30">
      ...
      case 'Processing':
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-semibold rounded-full bg-cyan-500/20 text-cyan-700 dark:text-cyan-400 border border-cyan-500/30">
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-semibold rounded-full bg-amber-500/20 text-amber-700 dark:text-amber-400 border border-amber-500/30">
                        ...
                        <div className="flex items-center justify-between gap-2 mb-3">
                          <span className={`px-2.5 py-1 rounded-lg text-xs font-bold uppercase tracking-wider border ${
                            prize.placement_rank === 1
                              ...
                              ? 'bg-slate-300/20 text-slate-700 dark:text-slate-300 border-slate-400/40'
                              : prize.placement_rank === 3
                              ? 'bg-amber-800/20 text-amber-700 dark:text-amber-400 border-amber-700/40'
                              : 'bg-slate-800/20 text-slate-600 dark:text-slate-400 border-slate-700/40'
                          }`}>
                            Juara #{prize.placement_rank} - {prize.title}
                          </span>
                          <span className="text-xs font-extrabold text-amber-600 dark:text-amber-400">
                            {event.currency || 'IDR'} {Number(prize.amount).toLocaleString()}
                          </span>
                        </div>

                        <div className="space-y-1">
                          <span className="text-[11px] font-semibold text-slate-400 uppercase block">Nama Pemenang / Tim:</span>
                          <h4 className={`text-base font-bold truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>
                            {prize.winner_name || <span className="text-slate-400 italic">Belum Ditentukan</span>}
                          </h4>
                        </div>
                      </div>

                      <div className="pt-3 border-t border-slate-800/60 flex items-center justify-between gap-2">
                        <span className="text-xs text-slate-400 font-medium">Status Payout:</span>
