...
- 017-multi-tier-input-sorting: Implemented system-wide multi-tier DESCENDING sorting (`transaction_date`/`start_date`/`joined_date` DESC -> `created_at` DESC -> `id` DESC) across ALL modules (`expenseRepository.js`, `incomeRepository.js`, `paymentRepository.js`, `eventRepository.js`, `eventController.js`, `financeController.js`, `loggerService.js`) ensuring newly created records immediately float to the top of table views across all menus.
- 016-event-category-rename-and-migration: Renamed expense category `Event Prize Payout` to **`Event`**, updated event winner prize auto-sync expense creation logic in `eventController.js`, updated default categories in `financeController.js` and `ExpensePage.jsx`, and safely migrated all stored records in `expenses.json` and `settings.json`.
- 015-searchable-recipient-select-in-expenses: Replaced standard recipient dropdown in `ExpensePage.jsx` modal form with a real-time **Searchable Recipient Select** component (supporting instant search by full name, IGN tag, bank account, and entity type) and added click-outside auto-dismiss listener (`useRef` + `mousedown`) to prevent dropdown overflow.
