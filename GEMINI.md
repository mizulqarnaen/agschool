...
## Recent Changes
- 017-multi-tier-input-sorting: Implemented multi-tier DESCENDING sorting (`transaction_date` DESC -> `created_at` DESC -> `id` DESC) across financial repositories (`expenseRepository.js`, `incomeRepository.js`, `paymentRepository.js`) ensuring newly created records immediately float to the top of table views.
